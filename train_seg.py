...
def main(config):
  ...
  for epoch in range(start_epoch, config['epoch'] + 1):
    ...
    for data in tqdm(train_loader, desc='meta-train', leave=False):
      ...
      segmentation = model(x_shot, x_query, y_shot, inner_args, meta_train=True)
      segmentation = segmentation.flatten(0, 1)
      labels = y_query.flatten()
      
      pred = segmentation
      acc = utils.compute_acc(pred, labels) # TODO TODO TODO TODO
      loss = F.mse_loss(segmentation, labels)
    ...
    if eval_val:
      ...
      for data in tqdm(val_loader, desc='meta-val', leave=False):
        ...
        segmentation = model(x_shot, x_query, y_shot, inner_args, meta_train=False)
        segmentation = segmentation.flatten(0, 1)
        labels = y_query.flatten()
