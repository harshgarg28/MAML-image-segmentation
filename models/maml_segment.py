...
class MAML(Module):
  ...
  def _inner_forward(self, x, params, episode):
    ...
    segmentation = self.classifier(feat, get_child_dict(params, 'classifier'))
    return segmentation
  ...
  def _inner_iter(self, x, y, params, mom_buffer, episode, inner_args, detach):
    ...
    with torch.enable_grad():
      # forward pass
      segmentation = self._inner_forward(x, params, episode)
      loss = F.mse_loss(segmentation, y)
      # backward pass
  ...
  def forward(self, x_shot, x_query, y_shot, inner_args, meta_train):
    ...
    for ep in range(x_shot.size(0)):
      ...
      with torch.set_grad_enabled(meta_train):
        self.eval()
        seg_ep = self._inner_forward(x_query[ep], updated_params, ep)
      segmentation.append(seg_ep)
